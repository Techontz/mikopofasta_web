...
export function BranchFormDialog({ branch, regions, zones, branches }: BranchFormDialogProps) {
  ...
  const defaults: FormValues = {
    name: branch?.name ?? "",
    /*
     * NOT ASKED FOR, and deliberately not sent.
     *
     * The branch code is generated by the API from the name
     * (BranchCodeGenerator), uniquified against every branch including
     * soft-deleted ones, and it lands in every customer payment reference —
     * MF-2026-KKO-000001. Nobody should have to invent one, and an editable box
     * invites somebody to change a code that references already point at.
     *
     * Undefined rather than "": the create request treats an absent code as
     * "derive it", and on edit UpdateBranchAction keeps the code the branch
     * already has rather than re-deriving a new one.
     */
    code: undefined,
